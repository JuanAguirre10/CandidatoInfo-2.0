...
function Diputados() {
  ...
  const [editingDiputado, setEditingDiputado] = useState(null);
  ...
  const [formData, setFormData] = useState({
    ...
    posicion_lista: '',
    numero_preferencial: '',
    es_natural_circunscripcion: false,
    estado: 'inscrito',
    fecha_inscripcion: '',
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingDiputado) {
        await updateDiputado(editingDiputado.id, formData);
      } else {
        await createDiputado(formData);
      }
      setShowModal(false);
      setEditingDiputado(null);
      resetForm();
      loadData();
    } catch (error) {
      if (error.response) {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('¿Está seguro de eliminar este diputado?')) {
      try {
        await deleteDiputado(id);
        loadData();
      } catch (error) {
        console.error('Error eliminando diputado:', error);
        alert('Error al eliminar diputado');
      }
    }
  };
  ...
  const handleExport = async () => {
    try {
      const response = await exportDiputados();
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `diputados_${new Date().getTime()}.xlsx`);
  ...
  const openModal = (diputado = null) => {
    if (diputado) {
      setEditingDiputado(diputado);
      setFormData(diputado);
    } else {
      setEditingDiputado(null);
      resetForm();
    }
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-gray-800">Diputados</h1>
                  ...
                  {diputados.map((dip) => (
                    <tr key={dip.id}>
                      <td className="px-6 py-4">
                        {dip.foto_url ? (
                          <img src={dip.foto_url} alt={dip.nombre} className="w-12 h-12 rounded-full object-cover" />
                        ) : (
                      ...
                      <td className="px-6 py-4">{dip.partido_siglas || '-'}</td>
                      <td className="px-6 py-4">{dip.circunscripcion_nombre || '-'}</td>
                      <td className="px-6 py-4">{dip.nombre} {dip.apellidos}</td>
                      <td className="px-6 py-4">{dip.dni}</td>
                      <td className="px-6 py-4">{dip.genero}</td>
                      <td className="px-6 py-4">{dip.profesion || '-'}</td>
                      <td className="px-6 py-4 text-center">{dip.posicion_lista}</td>
                      <td className="px-6 py-4 text-center">{dip.numero_preferencial || '-'}</td>
                      <td className="px-6 py-4">
                        <span className={`px-2 py-1 rounded-full text-xs ${
                          dip.estado === 'aprobado' ? 'bg-green-100 text-green-800' : 
                          dip.estado === 'observado' ? 'bg-yellow-100 text-yellow-800' : 
                          dip.estado === 'excluido' ? 'bg-red-100 text-red-800' :
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <button onClick={() => openModal(dip)} className="text-blue-600 hover:text-blue-800 mr-3">
                          <Edit size={18} />
                        </button>
                        <button onClick={() => handleDelete(dip.id)} className="text-red-600 hover:text-red-800">
                          <Trash2 size={18} />
                        </button>
                      </td>
                    </tr>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-8 max-w-4xl w-full max-h-screen overflow-y-auto">
            <h2 className="text-2xl font-bold mb-6">{editingDiputado ? 'Editar' : 'Nuevo'} Diputado</h2>
            <form onSubmit={handleSubmit}>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium mb-2">Circunscripción *</label>
                  <select
                    required
                ...
                <div className="col-span-2">
                  <label className="flex items-center space-x-2">
                    <input
                      type="checkbox"
                      checked={formData.es_natural_circunscripcion}
                      onChange={(e) => setFormData({ ...formData, es_natural_circunscripcion: e.target.checked })}
                      className="w-4 h-4 text-blue-600"
                    />
                    <span className="text-sm font-medium">Es natural de la circunscripción</span>
                  </label>
                </div>
                <div className="col-span-2">
