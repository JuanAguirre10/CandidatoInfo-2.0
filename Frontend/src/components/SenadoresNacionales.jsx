...
import {
  ...
  createSenadorNacional,
  updateSenadorNacional,
...
function SenadoresNacionales() {
  ...
  const cleanFormData = (data) => {
    ...
    const numericFields = ['edad', 'posicion_lista', 'numero_preferencial', 'partido'];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingCandidato) {
        await updateSenadorNacional(editingCandidato.id, dataToSend);
      } else {
        await createSenadorNacional(dataToSend);
      }
      setShowModal(false);
  ...
  const handleDelete = async (id) => {
    if (window.confirm('¿Está seguro de eliminar este candidato?')) {
      try {
        await deleteSenadorNacional(id);
        loadData();
      } catch (error) {
  ...
  const handleExport = async () => {
    try {
      const response = await exportSenadoresNacionales();
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `senadores_nacionales_${new Date().getTime()}.xlsx`);
  ...
  const handleImport = async (e) => {
    ...
    try {
      await importSenadoresNacionales(file);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-gray-800">Senadores Nacionales</h1>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-8 max-w-4xl w-full max-h-screen overflow-y-auto">
            <h2 className="text-2xl font-bold mb-6">{editingCandidato ? 'Editar' : 'Nuevo'} Senador Nacional</h2>
...
export default SenadoresNacionales;