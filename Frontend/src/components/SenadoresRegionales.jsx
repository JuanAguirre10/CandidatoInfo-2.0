...
import {
  ...
  createSenadorRegional,
  updateSenadorRegional,
...
function SenadoresRegionales() {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('¿Está seguro de eliminar este candidato?')) {
      try {
        await deleteSenadorRegional(id);
        loadData();
      } catch (error) {
  ...
  const handleExport = async () => {
    try {
      const response = await exportSenadoresRegionales();
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `senadores_regionales_${new Date().getTime()}.xlsx`);
  ...
  const handleImport = async (e) => {
    ...
    try {
      await importSenadoresRegionales(file);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-gray-800">Senadores Regionales</h1>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-8 max-w-4xl w-full max-h-screen overflow-y-auto">
            <h2 className="text-2xl font-bold mb-6">{editingCandidato ? 'Editar' : 'Nuevo'} Senador Regional</h2>
            <form onSubmit={handleSubmit}>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="col-span-2">
                  ...
                  />
                  {formData.foto_url && (
                    <img src={formData.foto_url} alt="Foto" className="mt-2 w-20 h-20 rounded-full object-cover border" />
                  )}
                </div>
                <div className="col-span-2">
...
export default SenadoresRegionales;